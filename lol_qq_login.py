...
loginButton = browser.find_element_by_xpath(
    '//*[@id="J_topUser"]/div[2]/div[2]/div[1]/em')
loginButton.click()
...
try:
    WebDriverWait(browser, 10, 0.5).until(
        EC.visibility_of_element_located(userPasswordLoginButtonLocator))
except Exception:
    browser.close()
    sys.exit(-1)

userPasswordLoginButton = browser.find_element_by_css_selector(
    '#switcher_plogin')
...
logoutTextLocator = (
    By.XPATH,
    '//*[@id="J_topUser"]/div[3]/div[2]/div[1]/a[2]')
try:
    WebDriverWait(browser, 10, 0.5).until(
        EC.visibility_of_element_located(logoutTextLocator))
except Exception:
    browser.close()
    sys.exit(-1)
...
# browser.save_screenshot("game.png")
qqCookie = browser.get_cookies()
# print browser.get_cookie("p_skey")
# print qqCookie
